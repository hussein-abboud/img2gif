# !filepath: src/preprocessing/split_sequencedataset.py
...
def split_sequence_dataset(fraction_of_data_to_be_used: float = 1.0) -> None:
    ...
    write_csv_dicts(SPLIT_DIR / "train_sequence.csv", train_rows)
    write_csv_dicts(SPLIT_DIR / "val_sequence.csv", val_rows)
    write_csv_dicts(SPLIT_DIR / "test_sequence.csv", test_rows)

    logging.info("\n[DONE] Stratified sequence splits created:")
    logging.info(f" → Train: {len(train_rows)}")
    logging.info(f" → Val:   {len(val_rows)}")
    logging.info(f" → Test:  {len(test_rows)}")

if __name__ == "__main__":
    split_sequence_dataset(fraction_of_data_to_be_used=1.0)
